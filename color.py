...
def AutoWhiteAdjust(n, f, clip, core):
    ...
    norm = max(blue, math.sqrt(red_corr**2 + green_corr**2 + blue_corr**2) / math.sqrt(3), small_number)
...
def tm(clip="",source_peak="",desat=50,lin=True,show_satmask=False,show_clipped=False ) :
    ...
    #luminance
    l=EXPR(clips=[r,g,b], expr="x 0.2627 *  y 0.678 * + z 0.0593 * +    ",format=vs.GRAYS)
    ...
    l2=EXPR(clips=[rl,gl,bl], expr="x 0.2627 *  y 0.678 * + z 0.0593 * +    ",format=vs.GRAYS)
    nl= l2
...
def tm_simple(clip="",source_peak="" ) :
    ...
    l=EXPR(clips=[l], expr="x 0 =  0.00001 x  ? ",format=vs.GRAYS)#avoid dividing by 0
