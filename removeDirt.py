...
def RemoveDirt(input: vs.VideoNode, repmode: int=16, remgrainmode: int=17, limit: int=10, tools=None) -> vs.VideoNode:
  ...
  sbegin = RG('ForwardClense')(input)
  send =  RG('BackwardClense')(input)
  removedirt = pick_tool(tools, 'removedirt', ('removedirt', 'rdvs')) == 'removedirt'
  if removedirt:
  ...
  alt = RG('Repair')(scenechange, input, mode=[repmode,repmode,1])
  restore = RG('Repair')(cleansed, input, mode=[repmode,repmode,1])
...
def RemoveDirtMC(input: vs.VideoNode, limit: int=6, repmode: int=16, remgrainmode: int=17, block_size: int=8, block_over: int=4, gpu: bool=False, tools=None) -> vs.VideoNode:
  ...
  clp = RemoveDirt(clp, repmode=2, remgrainmode=17, limit=limit, tools=tools)
